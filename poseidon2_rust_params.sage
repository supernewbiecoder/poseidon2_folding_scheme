# Remark: This script contains functionality for GF(2^n), but currently works only over GF(p)!
# A few small adaptations are needed for GF(2^n).
...
print("use pasta_curves::pallas::Scalar as Fr;")
print("use ff::PrimeField;")
print("use lazy_static::lazy_static;")
print()
print("/// Hàm tiện ích để dịch chuỗi Hex thành phần tử trường (Scalar)")
print("pub fn from_hex(s: &str) -> Fr {")
print("    let clean_s = s.trim_start_matches(\"0x\");")
print("    let bytes = hex::decode(clean_s).expect(\"Lỗi giải mã hex\");")
print("    let mut repr = [0u8; 32];")
print("    // Đảo ngược byte vì pasta_curves sử dụng little-endian")
print("    for (i, &b) in bytes.iter().rev().enumerate() {")
print("        repr[i] = b;")
print("    }")
print("    Fr::from_repr(repr).expect(\"Lỗi khởi tạo phần tử trường\")")
print("}")
print()
print("lazy_static! {")

# Ma trận đầy đủ (Full Matrix)
print("    pub static ref MAT_FULL: [[Fr; {}]; {}] = [".format(t, t))
for vec in MATRIX_FULL:
    print("        [", end="")
    for i, val in enumerate(vec):
        print("from_hex(\"{}\")".format(to_hex(val)), end="")
        if i < t - 1:
            print(", ", end="")
    print("],")
print("    ];")
...
# Ma trận một phần (Partial Matrix)
print("    pub static ref MAT_PARTIAL: [[Fr; {}]; {}] = [".format(t, t))
...
print("    pub static ref RC: Vec<[Fr; {}]> = vec![".format(t))
for i in range(0, len(round_constants), t):
    print("        [", end="")
    for j in range(t):
        if i + j < len(round_constants):
            print("from_hex(\"{}\")".format(to_hex(round_constants[i+j])), end="")
        else:
            print("from_hex(\"0x0\")", end="") # Padding nếu thiếu
        if j < t - 1:
            print(", ", end="")
    print("],")
print("    ];")
...
# Chạy thử Poseidon2 và in kết quả Test Vectors (Không xuất ra dạng Rust code nữa mà in ra log)
state_in  = vector([F(i) for i in range(t)])
state_out = poseidon2(state_in, MATRIX_FULL, MATRIX_PARTIAL, round_constants)

print("/*")
print("=== TEST VECTORS FOR RUST MOCK ===")
print("Input State:")
for i, val in enumerate(state_in):
     print("in_{}: {}".format(i, to_hex(val)))
print("\nExpected Output State:")
for i, val in enumerate(state_out):
     print("out_{}: {}".format(i, to_hex(val)))
print("*/")